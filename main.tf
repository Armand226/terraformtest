...
resource "mongodbatlas_cluster" "my_cluster" {
  ...
  provider_instance_size_name = "M10"    # You may change to a free-tier size if needed
  backing_provider_name      = "AZURE"
  region_name                = "US_WEST" # Correct argument for Azure region
}
...
resource "null_resource" "create_db_collection" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      until mongosh "${mongodbatlas_cluster.my_cluster.connection_strings[0].standard_srv}" --eval 'db.runCommand({ connectionStatus: 1 })'; do
        echo "MongoDB Atlas is not available yet. Retrying..."
        sleep 30  # Retry every 30 seconds
      done
      echo "MongoDB Atlas is available. Creating collection..."
